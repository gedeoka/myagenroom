<?php
class MsAgent_Model extends Model{
    ...
    public function msuList(){
        $sts=$this->db->prepare('select * from user where TypeID=3');
        $sts->execute();
        return $sts->fetchAll();
    }
    ...
    public function create($data){
        $sts=$this->db->prepare("INSERT INTO `user`(`userID`, `username`, `password`, `Name`, `TypeID`, `Date_Created`, `aktif`)            
                VALUES (:id,:user,md5(:password),:name,3,:dates,1)");
        $sts->execute(array(
            ':id'=>$data['id'],
            ':user'=>$data['uname'],
       ...
       //buat profilenya
        $sts2=$this->db->prepare("INSERT INTO `agent`
                (`AgentID`,`UserID`, `Name`, `Address`, `Phone`, `Fax`,`website`,`Date_Create`)            
                VALUES
                (:agentID,:userID, :Name,:Address,:Phone,:Fax,:website,:Date_Create)");
        $sts2->execute(array(
            ':agentID'=>  Code::profileid(),
            ':userID'=>$data['id'],
            ':Name'=>$data['name'],
            ':Address'=>'',
            ':Phone'=>'',
